...
> Учебный проект и примеры кода, а не подтверждение реализации или готовности к production. Границы транзакций, восстановление после сбоев и производительность нужно проверять на работающей реализации.
...
|-----------|----------|-------|--------|-------------------|
| **Pessimistic Lock** | `SELECT FOR UPDATE` | Сериализация изменений заблокированных строк | Блокирует строки, может быть deadlock, плохо под высокой нагрузкой | Средняя нагрузка, критично к консистентности |
| **Optimistic Lock** | Version column + `UPDATE WHERE version = ?` | Нет блокировки на время чтения/вычисления; UPDATE всё ещё блокирует | Требует retry-логики, может быть много конфликтов | Высокая нагрузка, невысокая конкуренция |
| **Atomic DB Ops** | `UPDATE SET count = count - 1 WHERE count > 0` | Один условный UPDATE; стоимость проверить замерами | Ограниченная логика, сложно с дополнительными действиями | Простые счетчики, не требуется доп. логика |
| **Redis** | `DECR` + watch | Малая стоимость операции на сервере; полную задержку измерить | Возможна потеря данных, нет транзакций с БД | Кэширование счетчика, временные акции |
| **Queue-based** | Очередь заказов + worker | Один активный consumer на ключ сериализует работу; повторы требуют идемпотентности | Задержка, сложность | Не требует синхронного ответа |

### 2.2. Учебный пример реализации на PostgreSQL
