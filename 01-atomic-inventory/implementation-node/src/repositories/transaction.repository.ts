...
export class TransactionRepository implements ITransactionRepository {
	...
	async findByRequestId(
		...
		console.log(
			`🔍 [TransactionRepository] findByRequestId called with requestId: ${requestId}`,
		);
	...
	async exists(requestId: string): Promise<boolean> {
		console.log(
			`🔍 [TransactionRepository] exists called with requestId: ${requestId}`,
		);
	...
	async getTotalDeducted(sku: string): Promise<number> {
		console.log(
			`🔍 [TransactionRepository] getTotalDeducted called with sku: ${sku}`,
		);
