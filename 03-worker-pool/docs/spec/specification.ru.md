...
> Учебный проект и фрагменты кода, а не текущая реализация или подтверждение готовности к production. Сейчас Reliable добавляет panic recovery, Advanced оборачивает Bounded. Таймауты, повторы, метрики, приоритеты и динамическое масштабирование здесь — план работ. Очередь в памяти не сохраняет задачи после падения процесса.
...
### 1.1. Почему неограниченный приём задач может перегрузить систему

**Проблема:** Горутины и асинхронная работа расходуют ресурсы. Исчерпают ли 100k задач память, зависит от стеков, данных, I/O и доступных ресурсов; универсального порога нет.

```go
// ПЛОХО - так делать в проде нельзя
func (s *Server) handleRequest(w http.ResponseWriter, r *http.Request) {
    // Число активных задач зависит от потока и длительности; приём здесь не ограничен.
...
- Всплеск нагрузки может исчерпать ресурсы БД/Redis и других зависимостей
...
## 2. Предлагаемая архитектура worker pool
...
5. **Shutdown deliberately** — задайте политику drain/cancel; для сохранения задач после падения нужны долговечное хранилище и восстановление
