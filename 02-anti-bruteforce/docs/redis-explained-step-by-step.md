...
Поэтому для bruteforce (один IP бьёт много раз) L3 не идеален. L4 (Lua) — один атомарный скрипт без retry из-за WATCH-конфликта; сетевые ошибки и неизвестный результат требуют отдельной обработки.
...
| L4 Atomic | Redis ZSET | Lua-скрипт без перемежения команд | Один вызов; стоимость и сбои измеряем |
