...
> Учебный справочник с примерами, а не проверенная production-реализация. Перед переносом примеров проверить доверенные прокси, единицы времени, уникальность запросов, учёт веса и поведение при сбоях Redis.
...
Самая частая ошибка — использование `req.remoteAddr` напрямую. В продакшене приложение часто стоит за прокси (nginx, ingress, Cloudflare).
...
| Алгоритм | Use Case | Плюсы | Минусы | Что проверить |
|----------|----------|-------|--------|------------|
| **Fixed Window** | Общие лимиты, мониторинг | Простой, минимальный overhead | Проблема с границами окон | Зависит от условий |
| **Sliding Window Log** | Антибрутфорс, точные лимиты | Максимально точный | Много памяти в Redis | Атомарность реализации и политика сбоев |
| **Sliding Window Counter** | API limits, высокая нагрузка | Компромисс точность/память | Меньше точности чем Log | Зависит от условий |
| **Token Bucket** | Traffic shaping, burst | Поддерживает burst, плавный | Сложнее отладка | Зависит от условий |
| **Leaky Bucket** | Стабильный outflow | Предсказуемый outflow | Не гибкий | Зависит от нагрузки |

### 3.2. Учебный Lua-скрипт для Redis (Sliding Window Log)

Пример считает элементы через `ZCARD`: параметр веса не реализует произвольную взвешенную квоту. Случайный суффикс не доказывает уникальность элемента; единицы времени также должны совпадать. Атомарность скрипта не исправляет эти ошибки подсчёта.
